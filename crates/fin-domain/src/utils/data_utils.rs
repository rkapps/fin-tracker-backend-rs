...
use crate::{ticker::{Ticker, TickerHistory}, utils::date_utils::same_date};
...
pub fn get_industry_embeddings(tickers: &Vec<Ticker>) -> Vec<(Ticker, Vec<f32>)>{
    tickers
        .into_iter()
        .filter_map(|t| t.industry_embedding.clone().map(|embedding| (t.clone(), embedding)))
        .collect()
}
...
pub fn get_overview_embeddings(tickers: &Vec<Ticker>) -> Vec<(Ticker, Vec<f32>)>{
    tickers
        .into_iter()
        .filter_map(|t| t.overview_embedding.clone().map(|embedding| (t.clone(), embedding)))
        .collect()
}
